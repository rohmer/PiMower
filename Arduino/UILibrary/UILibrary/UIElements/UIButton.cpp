...
void UIButton::drawBorder(Adafruit_RA8875 *lcd, sThemeSettings themeSettings)
{
	...
	if (themeSettings.cornerType == eButtonCornerType::Rounded)
	{
		uint8_t radius = 5;
...
void UIButton::setButtonText(std::string buttonText, sThemeSettings currentSettings)
{
	buttonTextObj = new UIText(elementArea, buttonText, theme, eThemeObjState::defaultState);
	buttonTextObj->Invalidate();
	this->childElements.push_back(buttonTextObj);
}