...
#define SD_PIN              5   // pin used for the microSD enable signal
...
#if (FT_813_ENABLE==1)
 //Serial.println("FT813");
 cmd_text(GD.w / 2, (GD.h / 2)+(2*(GD.h / 20)), 27, OPT_CENTERX, "FT813 en Teensy 3.2");
#endif

  cmd_text(GD.w / 2, GD.h / 2, 27, OPT_CENTERX, "Presiona el punto para calibrar la pantalla tactil");
  cmd_number((GD.w / 2)-(GD.w / 20), (GD.h / 2)+(GD.h / 20), 27, 0, GD.w);
  cmd_number((GD.w / 2)+(GD.w / 80), (GD.h / 2)+(GD.h / 20), 27, 0, GD.h);
  cmd_text(GD.w / 2, (GD.h / 2)+(GD.h / 20), 27, OPT_CENTERX, "x");
  cmd_calibrate();
  finish();
  cmd_loadidentity();
  cmd_dlstart();
