...
UIAlphaNumericEditor::UIAlphaNumericEditor(DriverBase &tft, std::string editorText, eUITextFont textFont,
...
void UIAlphaNumericEditor::Update()
{
	...
	for (int y = 0; y < keyboardRows; y++)
	{
		...
		{			
			...
			std::string ch(1, keyRow[x].character);
			tft.textWrite(xStart + keyWidth / 2, yStart + keyWidth / 2,
				textFont, textColor, 0, ch);
			xStart += keyWidth;
		}
		yStart += keySize;
	}
