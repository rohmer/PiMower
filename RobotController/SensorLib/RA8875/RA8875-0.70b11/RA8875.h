...
//------------- HARDWARE ------------------------------------------------------------
	...
	void    	displayOn(bool on);//turn diplay on/off
	void    	sleep(bool sleep);//put display in sleep or not
	...
	//void		clearMemory(bool full);//clear the RA8875 internal buffer (fully or current layer)
...
using Print::write;
 ...
 protected:
	...
	*/
	...
	#endif
 private:
	void bitWrite(volatile uint8_t &x, unsigned int n, bool b);
	...
	uint8_t						    _maxTouch;//5 on FT5206, 1 on resistive
			...
			bool					_calibrated;
			bool					_isCalibrated(void);//true if screen calibration it's present
		#endif
	#endif
	...
	bool 	_waitPoll(uint8_t r, uint8_t f);//from adafruit
