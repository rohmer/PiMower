...
void Adafruit_RA8875::pushPixels(uint32_t num, uint16_t p) {
	...
	wiringPiSPIDataRW(0, RA8875_DATAWRITE, sizeof(RA8875_DATAWRITE));

	unsigned char *outbuf = new unsigned char(2);
	outbuf[0] = (p >> 8);
	outbuf[1] = p;
	while (num--) {
		wiringPiSPIDataRW(0, outbuf, 2);
	}
	delete(outbuf);
	digitalWrite(_cs, HIGH);
}
...
void Adafruit_RA8875::drawPixel(int16_t x, int16_t y, uint16_t color)
{
	...
	digitalWrite(_cs, LOW);
	wiringPiSPIDataRW(0, RA8875_DATAWRITE, sizeof(RA8875_DATAWRITE));
	wiringPiSPIDataRW(0, (unsigned char *)(color >> 8), 1);
	wiringPiSPIDataRW(0, (unsigned char *)(color), sizeof(color));
	digitalWrite(_cs, HIGH);
}
...
void  Adafruit_RA8875::writeData(uint8_t d)
{
	...
	spi_begin();
	wiringPiSPIDataRW(0, RA8875_DATAWRITE, sizeof(RA8875_DATAWRITE));
	wiringPiSPIDataRW(0, (unsigned char *)d, sizeof((unsigned char *)d));
	spi_end();
	digitalWrite(_cs, HIGH);
}
...
uint8_t  Adafruit_RA8875::readData(void)
{
	...
	wiringPiSPIDataRW(0, (unsigned char *)RA8875_DATAREAD, sizeof((unsigned char *)RA8875_DATAREAD));
	uint8_t x = wiringPiSPIDataRW(0, 0x0, sizeof(0x0));
	spi_end();
...
void  Adafruit_RA8875::writeCommand(uint8_t d)
{
	...
	wiringPiSPIDataRW(0, (unsigned char *)RA8875_CMDWRITE, sizeof((unsigned char *)RA8875_CMDWRITE));
	wiringPiSPIDataRW(0, (unsigned char *)d, sizeof((unsigned char *)d));
	spi_end();
...
uint8_t  Adafruit_RA8875::readStatus(void)
{
	digitalWrite(_cs, LOW);
	spi_begin();
	wiringPiSPIDataRW(0, (unsigned char *)RA8875_CMDREAD, sizeof((unsigned char *)RA8875_CMDREAD));
	uint8_t x = wiringPiSPIDataRW(0, (unsigned char *)0x0, sizeof((unsigned char *)0x0));
	spi_end();
